...
extension DLPersonalController:DLPersonalHeadDelegate{
    func turnCoupon() {
        guard let adress = systemOption?.user_coupon , DLUserInfo.verifyToken() else {
            return
        }
        let p = DLMyCouponController.init(urlStr: "\(adress)?token=\(DLUserInfo.share.token ?? "")")
    ...
    func avaterClick() {
        ...
        let v = DLlPersonalGoodsGrid.init(userId: DLUserInfo.share.userModel?.user_id ?? "")
...
extension DLPersonalController:UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case 3:
            ...
            self.navigationController?.pushViewController(vc, animated: true)
        case 4:
            let vc = DLPersonalList.init(type: .relation, userId: DLUserInfo.share.userModel?.user_id ?? "")
            vc.navigationItem.title = "我关注的"
            vc.hidesBottomBarWhenPushed = true
            self.navigationController?.pushViewController(vc, animated: true)
...
//    }
    ...
    self.head.avater.setImageWith(URL.init(string:DLUserInfo.share.userModel?.avatar ?? ""), for: .normal, placeholder: UIImage.init(named: "avater_d"))
        ...
        self.head.couponLbl.text = DLUserInfo.share.userModel?.couponnum ?? "0"
