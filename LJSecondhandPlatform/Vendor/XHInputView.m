...
static XHInputView* _instance = nil;
...
+(void)showWithStyle:(InputViewStyle)style historyText:(NSString*)historyText configurationBlock:(void(^)(XHInputView *inputView))configurationBlock sendBlock:(BOOL(^)(NSString *text))sendBlock{
//    XHInputView *inputView = [[XHInputView alloc] initWithStyle:style];
    XHInputView *inputView = [[XHInputView alloc] init];
    inputView.style = style;
    inputView.historyText = historyText;
    [inputView setupUI];
    [[NSNotificationCenter defaultCenter] addObserver:inputView selector:@selector(keyboardWillAppear:) name:UIKeyboardWillShowNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:inputView selector:@selector(keyboardWillDisappear:) name:UIKeyboardWillHideNotification object:nil];
    UIWindow *window = [UIApplication sharedApplication].delegate.window;
    [window addSubview:inputView];
    if(configurationBlock) configurationBlock(inputView);
    inputView.sendBlcok = [sendBlock copy];
...
//    }
//    _textView.text = nil;
    _placeholderLab.hidden = _textView.text.length != 0;
...
-(void)setupUI{
    ...
    switch (_style) {
        case InputViewStyleDefault:{
            ...
            [_sendButton setTitleColor:[UIColor darkTextColor] forState:UIControlStateNormal];
            ...
            if (![NSString isEmpty:_historyText]) {
                _textView.text = _historyText;
            }
...
- (void)keyboardWillDisappear:(NSNotification *)noti{
    ...
    if(_textView.isFirstResponder){
        ...
        if ([self.delegate respondsToSelector:@selector(xhKeyBoardDidDisappear:)]) {
            [self.delegate xhKeyBoardDidDisappear:self.textView.text];
        }
